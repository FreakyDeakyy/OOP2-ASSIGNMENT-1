...
public class PersonViewController {
  ...
  protected void onSaveButtonClick() {
    try {
      this.aPerson = new Person(
        aNameTextField.getText(),
        aDOBDatePicker.getValue(),
        aEmailAddressTextField.getText()
      );
    ...
    Alert successAlert = new Alert(
      ...
      this.aPerson.toString() + " saved successfully!"
    );
  ...
  private void handleInputError(IllegalArgumentException pException) {
    Alert errorAlert = new Alert(
      Alert.AlertType.ERROR,
      "Entered data invalid: " + pException.getMessage()
    );
    errorAlert.showAndWait();
  }
}
