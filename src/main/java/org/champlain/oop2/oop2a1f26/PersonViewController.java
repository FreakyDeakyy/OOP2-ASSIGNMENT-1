...
/**
 * Controller class for handling user interactions in the Person View GUI. This
 * class manages the input, display, and interaction logic for a {@link Person}
 * object, including saving data, loading example data, and purchasing a parking
 * pass.
 */
public class PersonViewController {
  ...
  /**
   * Called when the "Save" button is clicked. Validates and saves the entered
   * person data into the {@link Person} object. If successful, displays a
   * confirmation dialog. If validation fails, displays an error alert.
   */
  @FXML
  ...
  /**
   * Called when the "Load Example" button is clicked. Loads a sample
   * {@link Person} with pre-defined values and displays it in the form. If
   * setting sample data fails (which should not normally happen), shows an
   * error.
   */
  @FXML
  ...
  /**
   * Called when the "Buy Pass" button is clicked. Attempts to purchase a
   * parking pass for the {@link Person}. If already owned, displays an error
   * alert.
   */
  @FXML
    ...
    if (this.aPerson == null) {
      ...
      if (!purchaseResult) {
        Alert errorAlert = new Alert(Alert.AlertType.ERROR,
            "This person already had a parking pass! Don't waste my money!");
        errorAlert.showAndWait();
      }
    }
  }
  ...
  /**
   * Updates the GUI form fields with the data from the given {@link Person}
   * object. Also updates the parking pass label with appropriate message and
   * color.
   *
  ...
  /**
   * Displays an error alert dialog with a message derived from the given
   * exception.
   *
   * @param pException The {@link IllegalArgumentException} that occurred due to
   * invalid input.
   */
